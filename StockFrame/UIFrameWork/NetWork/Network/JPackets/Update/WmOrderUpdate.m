...
#import "WmOrderUpdate.h"

@implementation WmOrderUpdateCore

@end

@implementation WmOrderUpdate
- (id)init
{
    self = [super init];
    if (self)
...
-(void)fillData:(NSDictionary *)dict{
    ...
    _from_get = [[dict objectForKey:@(5)] intValue];
    _account = [dict objectForKey:@(6)];

    NSMutableArray *arr = [NSMutableArray new];
    for (NSDictionary *dictData in d)
    {
        WmOrderUpdateCore *core = [WmOrderUpdateCore new];
        core.sym = dictData[@1];
